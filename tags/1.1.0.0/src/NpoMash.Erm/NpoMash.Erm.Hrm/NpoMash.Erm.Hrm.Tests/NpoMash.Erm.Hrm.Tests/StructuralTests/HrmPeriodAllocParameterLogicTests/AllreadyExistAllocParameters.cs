...
namespace NpoMash.Erm.Hrm.Tests.StructuralTests.HrmPeriodAllocParameterLogicTests {
    ...
    public class AllreadyExistAllocParameters : OptionsAllocParameters {
        ...
        public void AddTestData_StableState_ComplexExpext() {
            ...
            var alloc_parameters_list = test_object_space.GetObjects<HrmAllocParameter>();
            foreach (var alloc_parameter in alloc_parameters_list) {
                ...
                Assert.AreEqual(HrmAllocParameterLogic.INIT_NORM_NO_CONTROL_KB, alloc_parameter.NormNoControlKB);
                Assert.AreEqual(HrmAllocParameterLogic.INIT_NORM_NO_CONTROL_OZM, alloc_parameter.NormNoControlOZM);
