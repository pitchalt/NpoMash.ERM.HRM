...
namespace NpoMash.Erm.Hrm.Tests.StructuralTests {
    struct CellImportedFormExcel {
        public List<CellValue> cell_info;
        public List<CellValue> row_info;
        public List<CellValue> col_info;
        public List<CellValue> itog_row_info;
        public List<CellValue> itog_col_info;
        }

    static class ImportTestDataFromExcelLogic {

        static List<CellImportedFormExcel> GetData(String path_to_file, int itogs_in_row = 1, int itogs_in_col = 1) {
            ...
            while (ws.Cells[current_row, 0].Value != end_string) {
                ...
                while (ws.Cells[0, current_col].Value != end_string) {
                    ...
                    List<CellValue> cell_info = new List<CellValue>();
                    ...
                    CellImportedFormExcel c = new CellImportedFormExcel();
                    c.cell_info = cell_info;
                    c.col_info = col_info;
                    c.row_info = row_info;
                    c.itog_col_info = itog_col_info;
                    c.itog_row_info = itog_row_info;
                    result.Add(c);
                }
