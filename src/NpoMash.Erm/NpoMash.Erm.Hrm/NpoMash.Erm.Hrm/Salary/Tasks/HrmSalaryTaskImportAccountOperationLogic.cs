                ...
                if (current_row != null && current_column != null && !String.IsNullOrEmpty(file_payType) && current_row.Order != null && current_column.Department != null) {
                    ...
                    try {
                        if (orders_in_database.ContainsKey(file_order_code)) {
                            ...
                            local_task.LogRecord(LogRecordType.ERROR, null, null, "Не удалось подтвердить в базе наличие заказа с кодом: {" + account_operation.OrderCode + "}");
                            broken = true;
                        }
                        if (paytypes_in_database.ContainsKey(file_payType)) {
                            ...
                            local_task.LogRecord(LogRecordType.ERROR, null, null, "Не удалось подтвердить в базе наличие вида оплаты с кодом: {" + account_operation.PayTypeCode + "}");
                            broken = true;
                        }
                        if (departments_in_database.ContainsKey(file_department_code)) {
                            ...
                            local_task.LogRecord(LogRecordType.ERROR, null, null, "Не удалось подтвердить в базе наличие подразделения с бухкодом: {" + account_operation.DepartmentCode + "}");
                            broken = true;
                        }
                    }
                    catch (KeyNotFoundException) {
                        if (account_to_db.Order == null) local_task.LogRecord(LogRecordType.ERROR, null, null, "Не нашлось заказа с кодом: {" + file_order_code + "}"); 
                        if (account_to_db.PayType == null) local_task.LogRecord(LogRecordType.ERROR, null, null, "Не нашлось вида оплаты с кодом: {" + file_payType + "}"); 
                        if (account_to_db.Department == null) local_task.LogRecord(LogRecordType.ERROR, null, null, "Не нашлось подразделения с кодом: {" + file_department_code + "}"); 
                    ...
                    if (!broken) {
                        ...
                        local_task.LogRecord(LogRecordType.ERROR, null, null, "Не удалось создать ячейку матрицы из-за отсутствия подразделения и/или заказа и/или кода оплаты");
                        broken = true;
                    }
                }
            }
